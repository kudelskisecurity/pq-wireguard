...
func (device *Device) CreateMessageInitiation(peer *Peer) (*MessageInitiation, error) {
	...
	KDF2(&handshake.chainKey, &key, handshake.chainKey[:], sig1[:])
	//chainKey == C3
	//key == k3
...
func (device *Device) ConsumeMessageInitiation(msg *MessageInitiation) *Peer {
	...
	KDF2(&chainKey, &key, C2[:], msg.Sig1[:])
	//C3
	aead, _ := chacha20poly1305.New(key[:])
	_, err = aead.Open(hpeerPK[:0], ZeroNonce[:], msg.Static[:], hash[:])
	if err != nil {
		println("first AEAD not ok")
		return nil
	}
...
func (device *Device) ConsumeMessageResponse(msg *MessageResponse) *Peer {
	...
	ok := func() bool {
		...
		// verify identity
		verifier.Init(r, nil)
		isValid, _ := verifier.Verify(msg.Ct2[:], msg.Sig3[:], handshake.remoteStatic[:])
		if !isValid {
			return false
		}
		mixKey(&chainKey, &chainKey, msg.Sig3[:]) //c8
